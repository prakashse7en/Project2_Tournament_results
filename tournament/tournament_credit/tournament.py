...
def insertBye(idOfPlayer,tournament):
    ...
    c.execute(""" UPDATE player_tournament_stats SET (WINS) = ((player_tournament_stats.WINS + 1)) ,
                    (TOTAL_MATCHES) = ((player_tournament_stats.TOTAL_MATCHES + 1)) WHERE (PLAYER_ID=%s) and (tournament_id=%s)""", [(idOfPlayer),(tournament)])
    DB.commit()
    DB.close()
...
def reportWinner(tournamentId):
    ...
    c.execute("""select PLAYER_NAME from PLAYERS where P_ID= (select PLAYER_ID  from
                    player_tournament_stats where wins =(select MAX(wins) from player_tournament_stats) and
...
def restartSequence():
    """reintialises the id values of table player_tournament_stats,PLAYERS,FIXTURES,PLAYER_REGISTERED to 1 """
    DB = connect();
    c = DB.cursor()
...
def registerPlayerForTournament(playerName,tId):
    ...
    player = c.fetchall()
    c.execute("insert into PLAYER_REGISTERED(PLAYER_ID,TOURNAMENT_ID) VALUES (%s,%s)",[(player[0][0]),(tId)])
...
def countPlayers(tournamentId):
    ...
    DB = connect();
    c = DB.cursor()
    c.execute("select count(player_id) as players_count from PLAYER_REGISTERED where (TOURNAMENT_ID=%s);",[tournamentId])
    noOfRows = c.fetchone()
...
def playerStandings(tournamentId):
    ...
    c.execute("""select * from PLAYERS_STANDINGS WHERE (tournament_id=%s);""",[tournamentId])
    row = c.fetchall()
    DB.commit()
    DB.close()
    return row
...
def reportMatch(winner, loser,tId):
    ...
    c.execute (""" UPDATE player_tournament_stats SET (WINS) = ((player_tournament_stats.WINS + 1)) ,
                    (TOTAL_MATCHES) = ((player_tournament_stats.TOTAL_MATCHES + 1)) WHERE (PLAYER_ID=%s) """, [winner])
    c.execute (""" UPDATE player_tournament_stats SET (TOTAL_MATCHES) = ((player_tournament_stats.TOTAL_MATCHES + 1))
                    WHERE (PLAYER_ID=%s) """, [loser])


    c.execute (""" insert into FIXTURES(TEAM_1,TEAM_2,TOURNAMENT_ID) VALUES (%s,%s,%s)""",[(winner),(loser),(tId)])
    DB.commit()
    DB.close()
...
def distintWinsValue(tournamentId):
    ...
    c.execute("""select distinct wins from PLAYERS_WITH_COMMON_WINS where (TOURNAMENT_ID = %s)""",[tournamentId])
...
def playersStandingsOMWSimple(winNo,tournamentId):
    """get the player id based on win and tournament ids and get the corresponding matches played against from FIXTURES table
        for each player played against calculate his strength.

        Strength Calulation for player played against = total wins of opponent player's played against
    ...
    c = DB.cursor()
    """ PLAYERS_WITH_COMMONWINS is a view """
    c.execute("""select player_id from PLAYERS_WITH_COMMON_WINS  WHERE (wins=%s) and (TOURNAMENT_ID = %s) """, [(winNo),(tournamentId)])
    ...
    for i in range(0, len(playerIds)):
        c.execute("""select case when (team_1  = %s)  then team_2 else team_1 end
                    as opposing_team from fixtures where ((team_1  = %s) or (team_2 = %s)) and (TOURNAMENT_ID = %s); """, [(playerIds[i]),(playerIds[i]),(playerIds[i]),(tournamentId)])
        rows = c.fetchall()

        buffer_total_wins = 0
        playerIdFromI = 0
        for j in range(0,len(rows)):
            c.execute("""select wins from player_tournament_stats where (PLAYER_ID=%s) and (TOURNAMENT_ID = %s);  """, [(rows[j]),(tournamentId)])
        ...
        sortedValuesByWins = sorted(playersWinsDict.items(), key=itemgetter(1),reverse=True)   
...
def getRankBasedOnWins(tournamentId):
    ...
    DB = connect();
    c = DB.cursor()
    c.execute("""SELECT PLAYER_ID, rank() OVER (ORDER BY wins DESC) FROM player_tournament_stats where (tournament_id=%s);""",[tournamentId])
...
def getOpponent(playerId,tournamentId,playerToBeRemoved):
    """ return the opponent player Id based on total matches played ,wins and tournament
    Args:
    playerId - id of the player
    tournamentId: tournament id in which match is played
    playerToBeRemoved - id of the player whose value need to be removed from the list of values from fetched from db
    based on total matches played ,wins and tournament
    """
    DB = connect()
    c = DB.cursor()

    c.execute("""select total_matches from player_tournament_stats where (player_id = %s);""",[(playerId)])
    totalMatches = c.fetchall()
    c.execute("""select player_id from player_tournament_stats where wins in (select wins from player_tournament_stats where (player_id = %s) and (tournament_id=%s))
              and (player_id != %s) and (tournament_id=%s) and (total_matches=%s) ;""",[(playerId),(tournamentId),(playerId),(tournamentId),(totalMatches[0])])
    
    opponentIds = c.fetchall()

    if not opponentIds:
        opponentIds = getPlayerWithNearestWin(playerId,tournamentId,playerToBeRemoved,totalMatches)
...
def getPlayerWithNearestWin(playerId,tournamentId,playerToBeRemoved,totalMatches):
    """when no opponent is found for the player an opponent with nearest wins is picked and returned
     Args:
    playerId - id of the player
    tournamentId: tournament id in which match is played
    playerToBeRemoved - id of the player whose value need to be removed from the list of values from fetched from db
    ...
    DB = connect()
    c = DB.cursor()
    c.execute("""select player_id from player_tournament_stats where wins < (select wins from player_tournament_stats where (player_id = %s) and (tournament_id=%s))
              and (player_id != %s) and (tournament_id=%s) and (total_matches=%s) order by wins;""",[(playerId),(tournamentId),(playerId),(tournamentId),(totalMatches[0])])
    opponentIds = c.fetchall()
    DB.commit()
    DB.close()
    opponentIds = removeOpponentIds(playerToBeRemoved,opponentIds)       
...
def removeOpponentIds(playerToBeRemoved,opponentIds):
    """ remove the ids from opponentIds as this id has already played a match against him
    Args:
    playerToBeRemoved - id of the player whose value need to be removed from the list of values from fetched from db
    based on total matches played ,wins and tournament
...
def getPerfectOpponent(opponentIds,tournamentId):
    """ return the opponent player ids sorted based in their strength
        Strength = retreive players played against by each opponent id , total all the wins , compare with
...
def removeDuplicateOpponents(playerId,opponentIds,tournamentId):
    """ remove duplicate opponents with help of rows retreived from Fixtures based on player Id
    Args:    
    playerId - id of the player
    opponentIds - list of ids be played against the player
    tournamentId: tournament id in which match is played 
    ...
    c = DB.cursor()   
    c.execute("""select case when (team_1  = %s)  then team_2 else team_1 end
       as opposing_team from fixtures where ((team_1  = %s) or (team_2 = %s)) and (TOURNAMENT_ID = %s); """,
       [(playerId),(playerId),(playerId),(tournamentId)])
    rows = c.fetchall()
...
def getWinsInEntireTournament(playerId):
    ...
    c.execute("select sum(wins) from player_tournament_stats where (player_id=%s) ",[playerId])
    wins = c.fetchall()
    #print noOfRows[0]
