...
""" Contains methods to get winner of the tournament,get rank of each players at the end of tournament,
perform swiss pairings etc."""
...
def testStandingsBeforeMatches():
    """this method is used to test standings of players before match is started"""
    deleteMatches()
    deleteTournament()
    deletePlayers()
    #register the player tournament
    createTournament(1)
...
def getWinnerOfTournament():
    ...
    from second round players are played against opponents having similar match wins ,total matches and in same tournament.
    each round players are played against different set of players they are matched

    Duplicate opponents are removed player with nearby win value is chosen

    finally once all the rounds are completed player with maximum wins is reported"""
...
def getEligiblePlayer(sortedValuesByWins):
    """ when there are more than one opponent ranking 1st,this method will return a single opponent id from list of common opponents
    Args:
...
def oddNumberRegistration():
    ...
    here odd number of players is created and the first player is given bye and swiss pairing is tested for the same number of players"""
    deleteMatches()
    deleteTournament()
    deletePlayers()
    restartSequence()
    ...
    """ assuming first player is given bye and rounds---> first run the single round without first player
...
def rankBasedOnOMW():
    ...
    Here players with common wins are retreived and they are ranked based on OMW i.e ,total wins of each opponenets players played against
    is calculated and the total value is used to rank the players with common wins

    player inputs is used from getWinnerOfTournament()
    """ 
    ...
    #need to take length of disting wins and print standing among players with common wins
...
def displayFinalList(playersByRankList):
    """method is used to display the final postions of the players based on the wins
    Args:
    playersByRankList - orginal list of players fetched from db in which players with common winns are ordered
    """
...
def reorderPositions(playersByRankList,sortedValuesByWins):
    """method is used to reorder the positions of players with common wins based on values present in sortedValuesByWins
    Args:
    playersByRankList - orginal list of players fetched from db in which players with common winns are un ordered
