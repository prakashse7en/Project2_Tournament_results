...
3)Run sql files from either tournament_credit or tournament_basic path present 
  in remote desktop
4)Schema present in a sql file can be imported and executed using command -
  "psql -f <file path>/<file name>.sql" (where file path - is the path where the 
  file exists ,filename is tournament_swiss.sql).
5)To check if database schema is created properly ,type "psql" in command line to access 
  ...
  More details on psql commands for postgre can be found @ - 
